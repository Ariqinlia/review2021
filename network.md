...
[2xx]：请求成功处理
- 200，ok,请求被正常处理
- 204，no content,表示请求成功，但是响应的报文中不包含实体主体，就相当于只是个判断，只需要返回状态码就行了
- 206，partial content,只返回了请求资源的部分。这种情况一定有一个请求头[Range], 用来表示请求资源的范围，如果服务器接受了范围请求并且成功处理，就会返回206
[3xx]：请求需要附加的操作
- 301，move permanently, 表示资源被永久重定向了，这种情况会在响应头[Location]，用来表示新地址
- 302，found，表示资源被临时重定向了
- 303，这个和302很像，但是更加规范，明确告诉浏览器要使用get来访问location，其实在301和302都会默认改为get请求
- 304，not modified,表示资源没改变，可直接使用缓存，这里的缓存是协商缓存
- 307，跟302一样是临时重定向，但是解决了302会默认将post改为get的问题，严格限制不能从post改为get
[4xx]：一般表示客户端发生了错误
- 400，bad request,表示请求中存在语法错误，可能是参数传得不对，或者是请求方式不对等等
- 401，unauthorized,表示身份信息未通过验证
- 403，forbidden,表示禁止访问或者是没权限访问
- 404，not found,表示服务器没有找到响应的资源
[5xx]：表示服务端发生了错误
- 500，内部服务器错误
- 502，bad gateway, 有时候客户端不是直接向服务端发起请求的，这时候就存在代理服务器或者是网关，当代理服务器尝试执行请求时，收到前一个服务器的无效效应，就会返回502
- 503，service unavailable，服务器暂时无法使用，可能在升级或者维护
- 504，gateway timeout，请求超时

# http和https的区别
（1）http是超文本传输协议，信息是明文传输，https是通过ssl协议来加密传输，https比http安全
（2）https需要CA证书，需要一定的费用
（3）端口号不一样，http默认是80，https是443
（4）http连接比较简单，是无状态的；https则是由http+ssl构建的可进行加密传输、身份认证的网络协议，比http安全
# http1.0和http2.0的区别
（1）http2.0: 提升访问速度，请求资源所用时间更少，访问速度更快
（2）http2.0: 允许多路复用，允许同时通过单一的连接发送多重请求-响应信息。在http1.1中，浏览器在同一时间，针对同一域名下的请求有一定数量限制，超过限制会被阻塞
...
（3）添加本地缓存
（4）压缩资源文件
（5）将CSS样式表放在顶部，把JavaScript放在底部(根据浏览器的运行机制决定)
（6）避免使用CSS表达式
（7）减少DNS查询
（8）使用外部js和css
（9）避免重定向
